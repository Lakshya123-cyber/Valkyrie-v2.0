...
const suggestionState = StateField.define<string | null>({
  ...
  update(value, transaction) {
    ...
    // if we find setSuggestionEffect, return new value
...
const renderPlugin = ViewPlugin.fromClass(
  class {
    ...
    update(update: ViewUpdate) {
      // Rebuild decorations if doc changed, cursor moved or suggestions changed
    ...
    build(view: EditorView) {
      ...
      return Decoration.set([
        Decoration.widget({
          ...
          side: 1, // Rener after cursor (side: 1), not before (side: -1)
        }).range(cursor),
      ]);
    }
  },
  { decorations: (plugin) => plugin.decorations }, // Tell codemirror to use our deco
);
...
const acceptSuggestionKeymap = keymap.of([
  {
    ...
    run: (view) => {
      const suggestion = view.state.field(suggestionState);
      if (!suggestion) {
        return false; // No suggestion found
      }

      const cursor = view.state.selection.main.head;
      view.dispatch({
        changes: { from: cursor, insert: suggestion }, // insert suggested text
...
export const suggestion = (fileName: string) => [
  ...
  acceptSuggestionKeymap, // press tab to accept
];
